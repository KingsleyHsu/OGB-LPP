...
def local_neg_sample(pos_edges, num_nodes, num_neg, random_src=False):
    ...
    neg_src = neg_src.to(device=torch.device(f'cuda:1' if torch.cuda.is_available() else 'cpu'))
    neg_dst = neg_dst.to(device=torch.device(f'cuda:1' if torch.cuda.is_available() else 'cpu'))
